...
export function obsidianCommand(args: string[]) {
  ...
  if (subcommand === "open") {
    ...
    runObsidian(["open", ...noteTargetArgs(note)]);
  ...
  if (subcommand === "backlinks") {
    ...
    const result = runObsidian(["backlinks", ...noteTargetArgs(note), ...(json ? ["format=json"] : [])]);
  ...
  if (subcommand === "unresolved") {
    const json = args.includes("--json");
    const result = runObsidian(["unresolved", ...(json ? ["format=json"] : [])]);
    process.stdout.write(result.stdout);
    return;
  }

  if (subcommand === "orphans") {
    const result = runObsidian(["orphans"]);
    process.stdout.write(result.stdout);
    return;
  }

  if (subcommand === "deadends") {
    const result = runObsidian(["deadends"]);
    process.stdout.write(result.stdout);
    return;
  }
  ...
  if (subcommand === "property:set") {
    ...
    runObsidian(["property:set", ...noteTargetArgs(note), `name=${name}`, `value=${value}`]);
