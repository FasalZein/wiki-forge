...
export const STRIP_SUFFIXES = "service|handler|handlers|routes|controller|repository|module|middleware|guard|interceptor|pipe|filter|resolver|factory|provider|util|utils|helpers|constants|config|types|dto|entity|model|schema|validator|validators";
export const STRIP_DOTTED = new RegExp(`[.](${STRIP_SUFFIXES})$`, "u");
export const STRIP_HYPHEN = new RegExp(`-(${STRIP_SUFFIXES})$`, "u");

export function normalizeBasename(name: string) {
  return name
...
export function codeMatchKeys(file: string) {
...
export function testMatchKeys(file: string) {
  const normalized = file.replaceAll("\\", "/");
  const parts = normalized.split("/").filter(Boolean);
