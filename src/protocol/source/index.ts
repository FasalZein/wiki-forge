...
export type ProtocolCommandSurface = {
  plan: string;
  run: string;
  next: string;
  sessionStart: string;
...
export type CanonicalProtocolSource = {
  ...
  commandSurface: ProtocolCommandSurface;
...
export function buildCanonicalProtocolSource(project: string, scope: ProtocolScope): CanonicalProtocolSource {
  const commandSurface = {
    plan: `wiki forge plan ${project} <feature-name>`,
    run: `wiki forge run ${project} [slice-id] --repo <path>`,
    next: `wiki forge next ${project}`,
    sessionStart: `wiki resume ${project} --repo <path> --base <rev>`,
  };
  return {
    managedBy: "wiki-forge",
...
export function renderManagedProtocolBlock(source: CanonicalProtocolSource) {
  return [
    ...
    "",
    `Agent surface (3 commands): \`${source.commandSurface.plan}\`, \`${source.commandSurface.run}\`, \`${source.commandSurface.next}\``,
    `Session start: \`${source.commandSurface.sessionStart}\``,
    "",
    END_MARKER,
  ].join("\n");
}
...
export function renderPromptProtocolReminders(project: string) {
  ...
  return [
    ...
    `Plan tracked work with \`${source.commandSurface.plan}\`.`,
    `Run the active slice with \`${source.commandSurface.run}\`.`,
    `If no slice is active, use \`${source.commandSurface.next}\`.`,
  ];
}
