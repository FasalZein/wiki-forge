...
export function isV1ForgePath(path: VaultPath): boolean {
  const normalized = normalizePath(path);
  return /^projects\/[^/]+\/forge\/[^/]+(?:\/.*)?$/u.test(normalized);
}

export function isV1ForgeFeaturePath(path: VaultPath): boolean {
  return /^projects\/[^/]+\/forge\/features\/[^/]+\.md$/u.test(normalizePath(path));
}

export function isV1ForgePrdPath(path: VaultPath): boolean {
  return /^projects\/[^/]+\/forge\/prds\/[^/]+\.md$/u.test(normalizePath(path));
}

export function isV1ForgeSlicePath(path: VaultPath): boolean {
  return /^projects\/[^/]+\/forge\/slices\/[^/]+\/index\.md$/u.test(normalizePath(path));
}

export function isV1ForgeEvidencePath(path: VaultPath): boolean {
  return /^projects\/[^/]+\/forge\/evidence\/[^/]+\.md$/u.test(normalizePath(path));
}

export function isV1ForgeHandoverPath(path: VaultPath): boolean {
