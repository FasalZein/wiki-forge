...
export async function recordV1TddEvidence(input: RecordTddEvidenceInput): Promise<TddEvidenceRecord> {
  ...
  await appendV1Evidence(input.project, input.sliceId, record, input.vaultRoot);
  return record;
}

export async function recordV1VerificationEvidence(input: RecordVerificationEvidenceInput): Promise<VerificationEvidenceRecord> {
...
export async function recordV1ReviewEvidence(input: RecordReviewEvidenceInput): Promise<ReviewEvidenceRecord> {
...
export async function readV1Evidence(project: string, sliceId: string, vaultRoot = VAULT_ROOT): Promise<readonly V1EvidenceRecord[]> {
  ...
  return normalizeEvidenceList(parsed.data.v1_evidence);
}

async function appendV1Evidence(project: string, sliceId: string, record: V1EvidenceRecord, vaultRoot = VAULT_ROOT): Promise<void> {
  const path = sliceIndexPath(vaultRoot, project, sliceId);
  if (!existsSync(path)) throw new Error(`slice index not found: ${project}/${sliceId}`);
  const raw = await readFile(path, "utf8");
  const parsed = matter(raw);
  const existing = normalizeEvidenceList(parsed.data.v1_evidence);
  await writeFile(path, matter.stringify(parsed.content, {
    ...parsed.data,
    v1_evidence: [...existing, record],
  }), "utf8");
}

function normalizeEvidenceList(value: unknown): readonly V1EvidenceRecord[] {
  if (!Array.isArray(value)) return [];
  return value.flatMap((entry) => isEvidenceRecord(entry) ? [entry] : []);
}

function isEvidenceRecord(value: unknown): value is V1EvidenceRecord {
