...
3. Write the PRD using the template below, then publish it through `wiki forge plan <project> <feature-name> [--repo <path>]`.
...
For wiki-forge projects, the issue tracker is Forge. Preserve upstream PRD synthesis, but publish PRD/spec content through `wiki forge plan <project> <feature-name> --repo <path>` instead of creating external issues unless the user explicitly asks for external visibility.
