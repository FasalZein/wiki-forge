...
import { evaluateCloseSliceIntent } from "../../src/v1/forge/close-slice-intent";
...
describe("v1 dogfood release gate", () => {
  ...
  test("skills name lifecycle chain and subagent policy but defer enforcement to CLI/kernel", () => {
    ...
    expect(docs.forgeSkill).toContain("The CLI and V1 kernel own phase ordering, invariants, and close gates.");
  ...
  test("V1 dogfood fixture runs through next status and run/close with targeted verification", () => {
    ...
    const evidence: readonly V1EvidenceRecord[] = [
      { kind: "tdd", command: "bun test tests/v1/dogfood-release.test.ts", result: "passed", recordedAt: "2026-04-28T04:44:00.000Z" },
  ...
  test("release gate is explicit and separate from per-slice close", () => {
    ...
    expect(docs.setup).toContain("Run full `bun test` only for the production V1 release gate, not for normal per-slice closeout.");
  });
});
