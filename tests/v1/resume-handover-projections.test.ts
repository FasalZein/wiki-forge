...
import { buildAmendProjection } from "../../src/v1/projections/amend";
import { buildHandoverProjection } from "../../src/v1/projections/handover";
import { buildResumeProjection } from "../../src/v1/projections/resume";

describe("v1 resume handover amend projections", () => {
  test("resume packet cites status truth and does not mutate lifecycle state", () => {
    expect(buildResumeProjection({
      ...
      context: ["V1 rewrite in progress", "use targeted verification"],
    })).toEqual({
  ...
  test("handover contains target phase blockers and next command", () => {
    expect(buildHandoverProjection({
      ...
      nextCommand: "bun test tests/v1/resume-handover-projections.test.ts",
    })).toEqual({
