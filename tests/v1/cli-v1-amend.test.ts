...
describe("V1 forge amend", () => {
  test("legacy forge amend routes to V1 and ignores legacy backlog closure state", () => {
    const vault = tempDir("wiki-v1-amend-vault");
    initVault(vault);
    writeClosedV1Slice(vault, "demo", "DEMO-001", {
      status: "done",
  ...
  test("can start a V1 amendment without moving a legacy backlog row", () => {
    const vault = tempDir("wiki-v1-amend-start-vault");
    initVault(vault);
    writeClosedV1Slice(vault, "demo", "DEMO-001", {
      status: "done",
      sourcePaths: ["src/payments.ts"],
  ...
  test("refuses to amend slices that are not closed in V1 truth", () => {
    const vault = tempDir("wiki-v1-amend-open-vault");
    initVault(vault);
    writeClosedV1Slice(vault, "demo", "DEMO-001", {
      status: "in-progress",
      sourcePaths: [],
