...
describe("wiki sync reconciler", () => {
  test("report-only mode scopes reconciliation to directly impacted derived targets", () => {
    ...
    expect(json.writes.orientationTargets).toEqual([]);
    expect(json.writes.navigationTargets.length).toBeGreaterThan(0);
    expect(json.writes.total).toBeLessThan(10);
  });

  test("write mode applies only stale orientation render targets", () => {
    const { vault, repo } = setupVaultAndRepo();
    const env = { KNOWLEDGE_VAULT_ROOT: vault };
    ...
    expect(json.writes.orientationTargets).toContain("apps/api/AGENTS.md");
    expect(json.writes.orientationTargets).toContain("apps/api/CLAUDE.md");
