...
import { detectOwnershipOverlap } from "../../src/forge/kernel/ownership";
import { evaluateSubagentPolicy } from "../../src/forge/lifecycle/subagent-policy";

describe("forge subagent execution policy", () => {
  test("parallel implementation grants with overlapping files are rejected", () => {
    expect(detectOwnershipOverlap([
      { agentId: "worker-a", files: ["src/forge/forge/next-intent.ts"], slices: ["WIKI-FORGE-217"] },
      { agentId: "worker-b", files: ["src/forge/forge/next-intent.ts"], slices: ["WIKI-FORGE-218"] },
    ])).toEqual([
      {
        kind: "file-overlap",
        resource: "src/forge/forge/next-intent.ts",
    ...
    expect(evaluateSubagentPolicy({
      ...
      grants: [
        { agentId: "worker-a", mode: "implementation", files: ["src/forge/a.ts"], slices: ["WIKI-FORGE-216"], canMutateLifecycle: false },
        { agentId: "worker-b", mode: "implementation", files: ["src/forge/a.ts"], slices: ["WIKI-FORGE-216"], canMutateLifecycle: false },
      ],
    })).toEqual({
      status: "rejected",
      reason: "parallel implementation requires non-overlapping file and slice grants",
      overlaps: [
        { kind: "file-overlap", resource: "src/forge/a.ts", agentIds: ["worker-a", "worker-b"] },
  ...
  test("cross-phase parallelism is rejected", () => {
    expect(evaluateSubagentPolicy({
      ...
      grants: [
        ...
        { agentId: "worker", mode: "implementation", phase: "implementation", files: ["src/forge/x.ts"], slices: ["WIKI-FORGE-216"], canMutateLifecycle: false },
      ],
    })).toEqual({
      status: "rejected",
