...
describe("Forge Wiki memory note/log commands", () => {
  test("top-level note and log route to Forge typed memory", () => {
  ...
  test("note writes a typed project memory record without lifecycle fields", () => {
    ...
    const result = runWiki([
      "note",
      "demo",
      "Keep Forge memory separate from Forge lifecycle.",
      "--agent",
      "pi",
      "--slice",
      "DEMO-001",
    ...
    expect(markdown).toContain("Keep Forge memory separate from Forge lifecycle.");
  });
